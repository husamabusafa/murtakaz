...
export default function ApprovalsPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title={t("approvals")}
        subtitle={tr("Approval queue and history for KPI values.", "قائمة الاعتماد والسجل لقيم مؤشرات الأداء.")}
      ...
      <Card className="border-white/10 bg-white/5 text-white shadow-lg shadow-black/20">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            {tr("KPI approvals", "اعتمادات المؤشرات")}
          </CardTitle>
          <CardDescription className="text-slate-200">
            {tr("Submitted KPI values waiting for approval and full approval history.", "قيم مؤشرات مرسلة بانتظار الاعتماد وسجل الاعتمادات بالكامل.")}
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {loading ? (
                  ...
                  rows.map((row) => (
                    <TableRow key={row.id} className="border-white/10 hover:bg-white/5">
                      ...
                      <TableCell className="text-right text-slate-200">{String(row.status)}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
